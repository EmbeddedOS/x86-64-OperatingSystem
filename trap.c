...
void InterruptHandler(TrapFrame *tf)
{
    ...
    default: {
        if ((tf->cs & 3) == 3) {
            /* If the exception is generated by user mode, we force exit current
             * process. */
            printk("[Exception: %d][%x:%x]: Terminating process.\n",
                    tf->trapno,
                    ReadCR2(),
                    tf->rip);
            Exit();
        } else {
            /* If the exception is generated by kernel mode, we halt CPU. */
            char msg[70] = {0};
            sprintk(msg,
